...
var handler = async (m, { conn, command, args }) => {
  ...
  try {
    var img = await fetch(`https://api.betabotz.eu.org/api/tools/ssweb?url=${url}&apikey=${btc}`);
    if (!img) {
      await m.reply('Gagal saat percobaan pertama. Memulai percobaan kedua...');
      img = await fetch(`https://api.betabotz.eu.org/api/tools/ssweb?url=${url}&apikey=${btc}`);
